...
const pauseHandle = (id?: string) => {
  ...
  ElMessageBox.confirm(t("prompt.info", { handle: t("schedule.pause") }), t("prompt.title"), {
    ...
    .then(() => {
      baseService.put("/job/schedule/pause", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
...
const resumeHandle = (id?: string) => {
  ...
  ElMessageBox.confirm(t("prompt.info", { handle: t("schedule.resume") }), t("prompt.title"), {
    ...
    .then(() => {
      baseService.put("/job/schedule/resume", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
          message: t("prompt.success"),
          duration: 500,
          onClose: () => {
...
const runHandle = (id?: string) => {
  ...
  ElMessageBox.confirm(t("prompt.info", { handle: t("schedule.run") }), t("prompt.title"), {
    ...
    .then(() => {
      baseService.put("/job/schedule/run", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
          message: t("prompt.success"),
          duration: 500,
          onClose: () => {
